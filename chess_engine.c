            ...
            switch (piece[i])   {
                ...
                case KING:
                    ...
                    if (col && color[i - 1] == xside)
                        generate_push_king (i, i - 1, pBuf, &capscount);
                    ...
                    if (i > 7 && color[i - 8] == xside)
                        generate_push_king (i, i - 8, pBuf, &capscount);
                    ...
                    if (col && i > 7 && color[i - 9] == xside)
                        generate_push_king (i, i - 9, pBuf, &capscount);
                    
                    // right up
                    if (col < 7 && i > 7 && color[i - 7] == xside)
                        generate_push_king (i, i - 7, pBuf, &capscount);
...
static int eval() {
    ...
    for (i = 0; i < 64; i++) {
        if (color[i] == WHITE) {
            // In the current square, add the material value of the piece
            score += value_piece[piece[i]];

            // Now we add to the evaluation the value of the piece square tables
            switch (piece[i]) {
                case PAWN:
                    score += pst_pawn[i];
                    break;
        ...
        // Now the evaluation for black: note the change of the sign in the score
...
int is_attacked(int current_side, int k) {
    ...
    // Check diagonal lines for attacking of Queen, Bishop, King, Pawn go right down
    y = k + 9;
    if (y < 64 && COL (y) != 0) {
        if (color[y] == xside) {
...
void take_back() {
    ...
    // If pawn moved two squares in the former move, we have to restore the eps square
    if (hist[hdp - 1].m.type == MOVE_TYPE_PAWN_TWO) {
        if (side == WHITE) {
    ...
    // To remove the eps square after unmaking a pawn * moving two squares
